...
contract ExchangeAuction {
    mapping(address => mapping(uint256 => Auction)) public nftContractAuctions;
    mapping(address => uint256) failedTransferCredits;

    struct Auction {
        uint32 bidIncreasePercentage; // Teklif artış yüzdesi
        uint32 auctionBidPeriod; // Yeni tekli tekliflerin yapılacağı süre
        uint64 auctionEnd; // Teklif süresi
        uint128 minPrice; // Minimum fiyat
        uint128 buyNowPrice; // Satın alma fiyatı
        uint126 nftHighestBid; // Teklif edilen en yüksek fiyat
        address nftHighestBidder; // Teklif eden kişi
        address nftSeller; // Satıcı
        address whitelistedBuyer; // whitelistli alıcı
        address nftRecipient; // Alıcı
        address ERC20TOKEN; // Ödeme yapılacak token
        address[] feeRecipients; // Komisyon alacak kişiler
        uint32[] feePercentages; // Komisyon oranları
    }

    /** NFT satıcısı tarafından belirtilmediği takdirde kullanılan varsayılan değerler  **/
    ...
    /** Minimum fiyat, buyNowPrice'ın (eğer ayarlanmışsa) %80'i olmalıdır. */
    ...
    /**
     * Teklif tutarı ya buyNowPrice'a eşit ya da öncekinden daha yüksek olmalı
     *  belirtilen teklif artış yüzdesi kadar teklif yapılmalıdır.
     */
    ...
    /** En yüksek teklisi yapan kişinin bu NFT'yi alıp alamayacağını kontrol et */
    ...
    /**
     * Ödeme, satıcı tarafından belirtilen ERC20 token veya ETH ile yapılırsa kabul edilir.
     *  Henüz müzayedeye çıkmamış NFT'lere erken teklifler ETH cinsinden yapılmalıdır.
     */
    ...
    constructor() {
        ...
        defaultAuctionBidPeriod = 86400; //1 gün
        ...
        returns (bool)
    {
        ...
        /**
         * AuctionEnd 0 ise müzayede teknik olarak devam ediyor ama
         * minPrice henüz karşılanmamıştır.
         */
    ...
    /**
     * Bir teklifin yapılıp yapılmadığını kontrol et(Erken teklif senaryosunda geçerlidir)
     * Erken bir tekliften sonra müzayede oluşturulursa müzayede uygun şekilde başlar veya buyOutPrice'a eşit olursa
     * kapatılır.
     */

    function isABidMade(address _nftContractAddress, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
    ...
    /**
     * minPrice satıcı tarafından belirlenmişse, en yüksek teklifin bu fiyatı karşıladığını veya aştığını kontrol et.
     */
    ...
    /**
     * buyOutPrice fiyatı satıcı tarafından belirlenmişse, en yüksek teklifin bu fiyatı karşılayıp karşılamadığını kontrol edin.
     */
    function _isBuyNowPriceMeet(address _nftContractAddress, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
        uint128 buyOutPrice = nftContractAuctions[_nftContractAddress][_tokenId]
            .buyOutPrice;
        return
            buyOutPrice > 0 &&
            (nftContractAuctions[_nftContractAddress][_tokenId].nftHighestBid >=
    ...
    /**
     * NFT'nin satın alınması için bir teklifin geçerli olup olmadığını kontrol et.
     * Satış durumunda: teklifin buyNowPrice'ı karşılaması gerekir.
     * Bir açık artırma durumunda: teklifin önceki tekliften bir % daha yüksek olması gerekir.
     */
    function _doesBidMeetBidRequirements(
        ...
        uint128 buyNowPrice = nftContractAuctions[_nftContractAddress][_tokenId]
            .buyNowPrice;
        /**
         * buyNowPrice karşılanırsa, artış yüzdesini göz ardı et
         */
        if (buyNowPrice > 0) {
            buyNowPrice > 0 &&
        ...
        /**
         * NFT müzayede için hazırsa, teklifin önceki tekliften % daha yüksek olması gerekir
         */
    ...
    /**
     * buyNowPrice ayarlandıysa ancak minPrice ayarlanmadıysa NFT satışa çıkar.
     * NFT satışını tamamlamanın tek yolu buyNowPrice'a ulaşmaktır.
     */

    function _isSale(address _nftContractAddress, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
    ...
    function isWhitelistedSale(address _nftContractAddress, uint256 _tokenId)
        internal
        view
        returns (bool)
    {
    ...
    /**
     * Aşağıdaki durumlarda en yüksek teklif sahibinin NFT'yi satın almasına izin verilir:
     * NFT satıcısı tarafından beyaz listeye alınmış bir alıcı belirlenmez.
     * Aksi takdirde, en yüksek teklifi veren Whitelist'te ki alıcıya eşit olmalıdır.
     */
    ...
    /**
     * Toplam teklifin yüzdesini verir (ücret ödemelerini hesaplamak için kullanılır)
     */
    ...
    /**
     * NFT alıcısı için varsayılan değer en yüksek teklifi verendir
     */
    ...
    function _getNftRecipient(address _nftContractAddress, uint256 _tokenId)
        ...
        returns (address)
    {
        address nftRecipient = nftContractAuctions[_nftContractAddress][tokenId]
            .nftRecipient;
    ...
    /**
     *  Tüm müzayedeler ve beyaz listeye alınan satışlar için geçerli kurulum parametreleri
     * -> Ödeme için ERC20 Token (satıcı tarafından belirtilmişse): _erc20Token
     * -> minimum price : _minPrice
     * -> buyNowPrice : _buyNowPrice
     * -> NFT satıcı : _nftSeller
     * Başarılı bir müzayede/satış için ücret alıcıları ve ilgili yüzdeleri
     * */
    ...
    /**
     * Varsayılan teklif artış yüzdesini, varsayılan açık artırma teklifi döneminde kullanan bir açık artırma oluşturun
     */
    ...
    function createNewNftAuction(
        ...
        uint128 _minPrice,
        uint128 _buyNowPrice,
        uint32 _auctionBidPeriod, //bu, müzayedenin başka bir teklif gelene kadar sürdüğü zamandır.
        ...
        priceGreaterThanZero(_buyNowPrice)
    {
        ...
        //buyNowPrice'ın karşılanıp karşılanmadığını kontrol edin ve satışı sonlandırın, aksi takdirde erken teklifi tersine çevirin
    ...
    /**
     * ETH veya NFT satıcısı tarafından belirtilen bir ERC20 Token ile teklif verin.
     * Ek olarak, bir alıcı, bir NFT'nin satışını gerçekleştirmek için talep edilen fiyatı ödeyebilir.
     */
     function _makeBid(
        address _nftContractAddress,
        uint256 _tokenId,
        ...
        onlyApplicableBuyer(_nftContractAddress, _tokenId)
    {
     ...
     function _updateOngoingAuction(
        ...
        // minimum fiyat belirlenmedi, nft henüz açık artırmaya çıkmadı
    ...
    function _updateAuctionEnd(address _nftContractAddress, uint256 _tokenId)
        internal
    {
        // müzayede sonu her zaman şimdi + teklif dönemi olarak ayarlanır

/**
 *Submitted for verification at testnet.snowtrace.io on 2023-01-25
*/
...
error NFTMarketPlace__BidNotActive();
error NFTMarketPlace__BidNotActiveOrNotEnoughFunds();
error NftMarketPlace__RequiredPriceIsLower(address nftAddress, uint256 tokenId, uint256 price);

contract NFTExchangeV2 is ReentrancyGuard {
    // Type Structure
    struct Listing {
        uint256 price;
        address seller;
    ...
    function placeBid(address nftAddress , uint256 tokenId, uint256 bidAmount) external isListed(nftAddress , tokenId ) {
    // ...
        struct bid memory newBid = bid({
            bidder: msg.sender,
            isActive: true,
            bidAmount: bidAmount
        });
        s_bids[nftAddress][tokenId].push(newBid);
    // ...
}

function cancelBid(address nftAddress, uint256 tokenId) external {
    require(msg.sender == s_bids[nftAddress][tokenId][0].bidder, "You are not the highest bidder");
    s_bids[nftAddress][tokenId][0].isActive = false;
